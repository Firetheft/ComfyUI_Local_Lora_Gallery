...
async def get_preview_image(request):
    ...
    if not filename or not lora_name or ".." in filename or "/" in filename or "\\" in filename: return web.Response(status=403)
    try:
        lora_full_path = folder_paths.get_full_path("loras", lora_name)
        if not lora_full_path: return web.Response(status=404)
...
async def update_lora_metadata(request):
    try:
        data = await request.json()
        lora_name, tags = data.get("lora_name"), data.get("tags")
        if not lora_name: return web.json_response({"status": "error", "message": "Missing lora_name"}, status=400)
        metadata = load_metadata()
        if lora_name not in metadata: metadata[lora_name] = {}
        if tags is not None: metadata[lora_name]['tags'] = [str(tag).strip() for tag in tags if str(tag).strip()]
...
async def get_lora_selection(request):
    try:
        node_id = request.query.get('node_id')
        if not node_id: return web.json_response({"error": "node_id is required"}, status=400)
...
class BaseLoraGallery:
    ...
    def IS_CHANGED(cls, **kwargs):
        if os.path.exists(SELECTIONS_FILE): return os.path.getmtime(SELECTIONS_FILE)
        return float("inf")
...
class LocalLoraGallery(BaseLoraGallery):
    ...
    RETURN_TYPES, FUNCTION, CATEGORY = ("MODEL", "CLIP"), "load_loras", "📜Asset Gallery/Local"
    ...
    def load_loras(self, model, clip, unique_id, **kwargs):
        ...
        if use_nunchaku_loader:
            ...
            print("LocalLoraGallery: Detected Nunchaku model. Using NunchakuFluxLoraLoader.")
        else:
        ...
        print(f"LocalLoraGallery: Applied {applied_count} LoRAs using {'Nunchaku' if use_nunchaku_loader else 'Standard'} loader.")
...
class LocalLoraGalleryModelOnly(BaseLoraGallery):
    ...
    RETURN_TYPES, FUNCTION, CATEGORY = ("MODEL",), "load_loras", "📜Asset Gallery/Local"
    ...
    def load_loras(self, model, unique_id, **kwargs):
        ...
        if use_nunchaku_loader:
            loader_instance = NunchakuFluxLoraLoader()
            print("LocalLoraGalleryModelOnly: Detected Nunchaku model. Using NunchakuFluxLoraLoader.")
        else:
        ...
        for config in lora_configs:
            ...
            try:
                ...
                (current_model,) = loader_instance.load_lora(current_model, lora_name, strength_model) if use_nunchaku_loader else loader_instance.load_lora_model_only(current_model, lora_name, strength_model)
                
                applied_count += 1
            except Exception as e:
                print(f"LocalLoraGalleryModelOnly: Failed to load LoRA '{lora_name}': {e}")
        
        print(f"LocalLoraGalleryModelOnly: Applied {applied_count} LoRAs using {'Nunchaku' if use_nunchaku_loader else 'Standard'} loader.")
        return (current_model,)

NODE_CLASS_MAPPINGS = { "LocalLoraGallery": LocalLoraGallery, "LocalLoraGalleryModelOnly": LocalLoraGalleryModelOnly }
NODE_DISPLAY_NAME_MAPPINGS = { "LocalLoraGallery": "Local Lora Gallery", "LocalLoraGalleryModelOnly": "Local Lora Gallery (Model Only)" }